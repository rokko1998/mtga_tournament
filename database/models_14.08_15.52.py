...
class TournamentStatus(enum.Enum):
    PLANNED = "planned"
    UPCOMING = "upcoming" # Турнир запланирован, но ещё не начался. Сбор участников
    ONGOING = "ongoing" # Турнир в процессе, игры проводятся.
    COMPLETED = "completed" # Турнир завершён, все матчи сыграны и результаты зафиксированы.
    CANCELLED = "cancelled" # Турнир был отменён. Не набралось нужное количество игроков
...
class UserORM(Base):
    ...
    accounts: Mapped[list['MtgORM']] = Relationship('MtgORM', back_populates='user', cascade="all, delete-orphan") # Связь с аккаунтами
...
class MtgORM(Base):
    ...
    # Relationships
    user_id: Mapped[int] = mapped_column(ForeignKey('users.id'), nullable=False) # Внешний ключ на пользователя
    user: Mapped['UserORM'] = Relationship('UserORM', back_populates='accounts') # Связь с UserORM
